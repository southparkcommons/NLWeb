...
The user's question is: {request.query}. The item's description is {item.description}""",
    ...
    def __init__(self, handler, items, ranking_type=FAST_TRACK):
        ll = len(items)
        self.ranking_type_str = (
            "FAST_TRACK" if ranking_type == self.FAST_TRACK else "REGULAR_TRACK"
        )
        logger.info(
    ...
    async def rankItem(self, url, json_str, name, site):
        ...
        try:
            ...
            ranking = await ask_llm(
                prompt, ans_struc, level="low", query_params=self.handler.query_params
            )
            logger.debug(
                f"Received ranking score: {ranking.get('score', 'N/A')} for item: {name}"
            )
